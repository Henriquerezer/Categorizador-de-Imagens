...
def process_images(image_dir, collection, model, processor, device):
    ...
    if not os.path.exists(image_dir):
        st.error(f"Diretório de imagens não encontrado: {image_dir}")
        return 0, 0, 0
    ...
    # Listar apenas os arquivos que existem e são imagens
    image_list = []
    ...
    if total_images == 0:
        st.warning(f"Nenhuma imagem encontrada no diretório: {image_dir}")
        return 0, 0, 0
...
def find_most_similar_images(query, collection, model, processor, device, n_results=4):
    if collection.count() == 0:
        st.error("Banco de dados vazio. Por favor, adicione imagens primeiro.")
        return [], [], []
...
# Função para upload de arquivo zip contendo imagens
def handle_uploaded_zip(upload_file):
    if upload_file is not None:
        ...
        # Extrair o conteúdo
        images_dir = os.path.join(temp_dir, "Imagens")
...
def main_app():
    ...
    # Usar diretórios relativos ao script
    base_dir = os.path.dirname(os.path.abspath(__file__))
    default_images_dir = os.path.join(base_dir, "Imagens")
    default_chroma_dir = os.path.join(base_dir, "chroma_db")
    
    # Inicializar diretórios se não existirem
    os.makedirs(default_images_dir, exist_ok=True)
    os.makedirs(default_chroma_dir, exist_ok=True)
    
    # Opção para upload de imagens por ZIP
    st.subheader("Upload de Imagens")
    upload_mode = st.radio("Escolha como fornecer imagens:", ["Usar diretório no servidor", "Upload por arquivo ZIP"])
    
    image_dir = default_images_dir
    chroma_dir = default_chroma_dir
    
    if upload_mode == "Usar diretório no servidor":
        # Entrada de pastas
        image_dir = st.text_input("📁 Caminho da pasta de imagens:", value=default_images_dir)
        chroma_dir = st.text_input("📁 Caminho para o ChromaDB:", value=default_chroma_dir)
    else:
        # Upload de arquivo ZIP
        uploaded_file = st.file_uploader("Carregar arquivo ZIP com imagens", type="zip")
        if uploaded_file is not None:
            image_dir = handle_uploaded_zip(uploaded_file)
            st.success(f"Arquivo ZIP extraído com sucesso! {len(os.listdir(image_dir))} arquivos encontrados.")

    # Debug: Mostrar o diretório atual
    st.info(f"Diretório atual: {os.getcwd()}")
    st.info(f"Diretório de imagens: {image_dir}")
    st.info(f"Diretório do ChromaDB: {chroma_dir}")
    ...
    if 'confirmed' in st.session_state:
        ...
        if st.sidebar.button("Buscar") and query:
            with st.spinner("Buscando imagens semelhantes..."):
                ...
                if image_ids:
                    ...
                    num_cols = min(4, len(image_ids))
                    rows = (len(image_ids) + num_cols - 1) // num_cols
                    ...
                    for row in range(rows):
                        ...
                        for col_idx in range(num_cols):
                            ...
                            if idx < len(image_ids):
                                with cols[col_idx]:
                                    try:
                                        img_path = metadatas[idx]["path"]
                                        if os.path.exists(img_path):
                                            img = Image.open(img_path)
                                            st.image(img, caption=f"{image_ids[idx]}\nSimilaridade: {1-distances[idx]:.2f}", use_container_width=True)
                                        else:
